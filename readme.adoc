...
- Task 2.2: The personal assistant agents are now expressing their votes, but still nothing is happening. That is becuase the voting is never closed. See the `TODO` on `line 33` of link:lab3/smart-room-vm/src/agt/room_controller.asl[room_controller.asl].

To solve these tasks, you will need to use the CArtAgO API for working with artifacts in Jason. Tips for a quick start:

* you will need to use the `focus` operation and to invoke artifact operations defined by the voting machine
...
* to create an instance of a `DweetArtifact` after creating the voting machine

* to link the voting machine to the created `DweetArtifact`` using the `linkArtifacts(ArtId1, "port", ArtId2)` operation
